...
def repeats(our_str):
    """
    Функция, которая добавляет к символам строки с помощью постфикса _n, сколько раз каждый символ уже встречался
    :param our_str: входная строка
    :return: новая строка, где к символам строки с помощью _n добавлено, сколько раз каждый символ уже встречался
    """
    new_str = ""
    for symbol in our_str:
        new_str += symbol + "_" + str(new_str.count(symbol) + 1)
    return new_str
